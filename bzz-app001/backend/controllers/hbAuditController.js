...
const endpoints = methods.crudController("SA_HB_WQAudit");
...
const Model = "SA_HB_WQAudit";
const ColumnModel = 'SA_HB_WQAudit_Columns'
const Logger_Model = 'SA_HB_WQAuditLogger'
...
io.get().then(i => {
  i.on('connection' , (socket) => {
      ...
      socket.on("HB-Audit-data", async (data)=>{

        const {recordset : result} = await sql.query(`
        SELECT  
        (
          SELECT COUNT(*) as [Total] FROM  ${Model} Where [Correct] IS NULL 
          ) as [Total],
        (
            SELECT COUNT(*) as [Done] FROM ${Model} Where [Correct] IS NOT NULL  and CAST([ActionTimeStamp] as  Date) = '${getDateTime().split('T')[0]}'
        ) as [Done]
          
        `)

          socket.emit('on-HB-Audit-data',{
            result
          })
      })
...
endpoints.filters = async (req, res) => {
    ...
    try {
        ...
        columns = columns.filter((column) => column['COLUMN_NAME'] == "Biller Answer"  ||  column['COLUMN_NAME'] == 'User Reviewed' ||  column['COLUMN_NAME'] == 'MRN' ||   column['COLUMN_NAME'] == 'HAR'  ||  column['COLUMN_NAME'] == 'IRB Reviewed')
...
endpoints.KPI = async (req, res,) => {
  try {
      ...
      let [{recordset: kpi}, {recordset: total}] = await Promise.all([
          await sql.query(`
          SELECT COUNT([Correct]) from ${Model} where [Correct] = 'Yes'
          UNION ALL   
          SELECT COUNT([Correct]) from ${Model} where [Correct] = 'No'
          `),
...
endpoints.update = async (req,res) => {
    try {
      ...
      let valuesQuery = "";
      for (key in values) {
        if(values[key] == null || values[key] == 'null' || values[key] == '' || values[key] == undefined) {
  ...
  endpoints.exports = async (req,res) => {
    try {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'WQ');
