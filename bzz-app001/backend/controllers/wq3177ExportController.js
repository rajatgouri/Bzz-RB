  ...
  const getObject = (obj) => {
    return  obj.map((o) => {
         ...
         return {
            ...
            'Date of Service': o['Date of Service'] ? o['Date of Service'].toISOString().split('T')[0] : '',
            'Patient MRN': o['Patient MRN'] ? o['Patient MRN'].toString() : '',
            'Status': o['Status'] ? o['Status'].toString() : '',
            'Patient Name': o['Patient Name'] ? o['Patient Name'].toString().replace(/'/g, "''") : '',
            'IRB Research Study No': o['IRB Research Study No'] ? o['IRB Research Study No'].toString() : '',
            'CPT Code': o['CPT Code'] ? o['CPT Code'].toString() : '',
            'Amount Due': o['Amount Due'] ? o['Amount Due'].toString() : '',
            'Account #': o['Account #'] ? o['Account #'].toString().replace(/'/g, "''") : '',
            'Account Type': o['Account Type'] ? o['Account Type'].toString().replace(/'/g, "''") : '',
            'Days Until Timely Filing Deadline': o['Days Until Timely Filing Deadline'] ? o['Days Until Timely Filing Deadline'].toString() : '',
            'Account Name': o['Account Name'] ? o['Account Name'].toString().split('T')[0] : '',
            'WQ Status': o['WQ Status'] ? o['WQ Status'].toString().split('T')[0] : '',
            'Invoice Number': o['Invoice Number'] ? o['Invoice Number'].toString() : '',
            'Payor Name': o['Payor Name'] ? o['Payor Name'].toString() : '',
            'Provider First Name': o['Provider First Name'] ? o['Provider First Name'].toString() : '',
            'Provider Last Name': o['Provider Last Name'] ? o['Provider Last Name'].toString() : '',
            'Service Facility Location': o['Service Facility Location'] ? o['Service Facility Location'].toString() : '',
            'User Error Message': o['User Error Message'] ? o['User Error Message'].toString() : '',
            'Date Created': o['Date Created'] ? o['Date Created'].toString() : '',
            '# Errors': o['# Errors'] ? o['# Errors'].toString() : '',
            'Specialty': o['Specialty'] ? o['Specialty'].toString() : '',
 ...
 endpoints.exports = async (req,res) => {
    try {
       ...
       let data = await getObject(objects1)
        ...
        let file = `WQ3177_SmartApp_${utilController.getDateTime().toString().replace(/-/g, '_').replace(/:/g, '_').split('.')[0]}.xlsx`
