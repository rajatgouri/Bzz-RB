...
io.get().then(i => {
    i.on('connection' , (socket) => {
        ...
        socket.on("WQ5508-process-start",(data)=>{
            i.sockets.emit('WQ5508-process-started',{
                id:data.id
            })
        })

        socket.on("WQ5508-process-end",(data)=>{
            i.sockets.emit('WQ5508-process-ended',{
                id:data.id
            })
        })
    })
    ...
    try {
        ...
        if (managementAccess) {
            const [{ recordset: Done }, { recordset: Pending }, { recordset: Defer }, { recordset: MiscI }, { recordset: MiscII }, { recordset: Review }] = await Promise.all([
                ...
                (filter['Process Type'] && filter['Process Type'][0] == 'Do Not Scrub IRBs') ?
    ...
    try {
        ...
        for (key in values) {
          if (values[key] === "null" || values[key] === null || values[key] === undefined || values[key].length == 0) {
           ...
           }  else if (values[key] == 'Expedite' ||  values[key] == 'Standard') {
...
where [SCRUB STATUS] = '0'
        ...
        if (managementAccess) {
            ...
            const [{ recordset: chargesProcessedCount }, { recordset: chargesReviewCount }, { recordset: chargesReview }, { recordset: notToReview },{ recordset: inReview }, { recordset: total }, { recordset: amount }, { recordset: charges }] = await Promise.all([
                ...
                await sql.query(`Select count(*) as count from ${Model} where Status IN ('Review') and  ([Research IRB]  NOT IN  (${scrubIRBs.map((irb) => "'" + irb + "'")}) or  [Research IRB] IS NULL)`),
            ...
            const [{ recordset: chargesProcessedCount }, { recordset: chargesReviewCount }, { recordset: chargesReview }, { recordset: notToReview }, { recordset: inReview }, { recordset: total }, { recordset: amount }, { recordset: charges }] = await Promise.all([
                ...
                await sql.query(`Select count([Sess Amount]) as count from ${Model} where Status NOT In ('Review')  and [User] IN ('${First}') and [ActionTimeStamp] > '${utilController.getDateTime().split('T')[0]} '`)
        ...
        io.get().then(i=>{
            i.to(req.admin.EMPID).emit('WQ5508-fulllist', {data: result1})
        })
