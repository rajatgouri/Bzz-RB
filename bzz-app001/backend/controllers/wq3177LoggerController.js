...
const endpoints = methods.crudController("WQ3177Logger");
const utilController = require('./utilController');
const writeXlsxFile = require('write-excel-file/node')

var sql = require("mssql");

const Model = "WQ3177Logger";
...
endpoints.create = async (req, res) => {
    try {
        ...
        if (typeof values.IDWQ3177 == 'number') {
...
;WITH CTELoggerData3177
AS
(
	SELECT ROW_NUMBER() OVER (PARTITION BY MRN, CONVERT(DATE, [DateTime]), [STATUS] ORDER BY MRN) Rno
	, IDWQ3177Logger, IDWQ3177, UserName, Color, MRN, [Status], CONVERT(DATE, DateTime) EntryDate
...
SELECT UserName, EntryDate, '3177' WQ, MIN(StartTime) StartTime, MAX(EndTime) EndTime, DATEDIFF(HOUR, MIN(StartTime), MAX(EndTime)) Duration , (SUM(DurationMin)) [Hours] 
FROM CTEMRNDuration3177
	WHERE StartTime IS NOT NULL
...
endpoints.exports = async (req, res) => {
    try {
        ...
        const schema = [
            {
                column: 'IDWQ3177Logger',
                type: String,
                value: wq => wq['IDWQ3177Logger'] ? wq['IDWQ3177Logger'].toString() : ''
            },
            {
                column: 'IDWQ3177',
                type: String,
                value: wq => wq['IDWQ3177'] ? wq['IDWQ3177'].toString() : ''
            },
            {
                column: 'UserName',
                type: String,
          ...
          objects1 = objects1.map((o) => {
            ...
            return  {
                'IDWQ3177Logger': o['IDWQ3177Logger'] ? o['IDWQ3177Logger'] .toString() : '',
                'IDWQ3177': o['IDWQ3177'] ? o['IDWQ3177'] .toString() : '',
          ...
          let file = `WQ3177Logger_SmartApp_${utilController.getDateTime().toString().replace(/-/g, '_').replace(/:/g, '.').replace(/.480Z/g, '')}.xlsx`
