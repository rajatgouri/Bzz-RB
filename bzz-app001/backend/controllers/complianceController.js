...
const endpoints = methods.crudController("Compliance");
...
endpoints.list = async (req, res) => {
  try {
    ...
    // let finalResult = result1.concat(result4)
...
endpoints.users = async (req, res) => {
  try {
    
    const {EMPID, date} = req.query
    const { recordset } = await sql.query(
      `select DISTINCT([FIRST_NAME]), EMPID from ${Model}`
    );
