...
const endpoints = methods.crudController("DoNotScrubIRBs");
var sql = require("mssql");
const { getDateTime } = require("./utilController");

const Model = "DoNotScrubIRBs";
...
endpoints.list = async (req, res) => {
  try {
    ...
    var query = `select ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo, * from ${Model} Left JOIN JWT WITH (NOLOCK)  ON ${Model}.EMPID = JWT.EMPID  `;
    ...
    if (filterQuery || sorterQuery) {
      ...
      query += `ORDER BY ${Model}.LastUpdated desc  OFFSET ` + skip + " ROWS FETCH NEXT " + limit + " ROWS ONLY "
    }
...
endpoints.create = async (req, res) => {
    try {
      ...
      let valuesQuery = "";
      values['LastUpdated'] =getDateTime()
      for (key in values) {
            if (values[key] === "null") {
              valuesQuery += "NULL" + ",";
            } else {
          ...
          let {recordsets} = await sql.query(`Select * from ${Model} where IRB like '${values.IRB}'`);
