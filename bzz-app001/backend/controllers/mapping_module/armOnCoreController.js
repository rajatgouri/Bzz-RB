...
const rb_ARM_Mapping_Model = '[SA_ARMMapping_OnCore]'
const Logger_Model = '[ARMMapping_OnCoreLogger]'
...
endpoints.list = async (req, res) => {
  try {
    ...
    Promise.all([
      ...
      select Distinct([TP_NAME]), [OnCore Arm] [Selected MCA], t2.[Notes] from ${Model} t1 WITH (NOLOCK)  Left JOIN ${ARM_Mapping_Model} t2 on t1.[TP_NAME] = t2.[EPIC ARM] where [IRB_APPROVAL_NUM]= '${virb}'`),
      ...
      await sql.query(`SELECT DISTINCT [EVENT_VISIT_ARM_DESCRIPTION] as SheetName
    ...
    if(!MCA_ARM.map(m => m['SheetName']).includes(null)) {
