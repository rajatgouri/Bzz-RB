...
const endpoints = methods.crudController("WQ1075Logger");
const utilController = require('./utilController');
var sql = require("mssql");
const writeXlsxFile = require('write-excel-file/node')


const Model = "WQ1075Logger";
...
endpoints.create = async (req, res) => {
    try {
        ...
        if (typeof values.IDWQ1075 == 'number') {
...
(
	...
	, IDWQ1075Logger, IDWQ1075, UserName, Color, MRN, [Status], CONVERT(DATE, DateTime) EntryDate
...
SELECT UserName, EntryDate, '1075' WQ, MIN(StartTime) StartTime, MAX(EndTime) EndTime, DATEDIFF(HOUR, MIN(StartTime), MAX(EndTime)) Duration , (SUM(DurationMin)) [Hours] 
FROM CTEMRNDuration1075
	WHERE StartTime IS NOT NULL
...
endpoints.exports = async (req, res) => {
    try {
        ...
        const schema = [
            {
                column: 'IDWQ1075Logger',
                type: String,
                value: wq => wq['IDWQ1075Logger'] ? wq['IDWQ1075Logger'].toString() : ''
            },
            {
                column: 'IDWQ1075',
                type: String,
                value: wq => wq['IDWQ1075'] ? wq['IDWQ1075'].toString() : ''
            },
            {
                column: 'UserName',
                type: String,
          ...
          objects1 = objects1.map((o) => {
            ...
            return  {
                'IDWQ1075Logger': o['IDWQ1075Logger'] ? o['IDWQ1075Logger'] .toString() : '',
                'IDWQ1075': o['IDWQ1075'] ? o['IDWQ1075'] .toString() : '',
          ...
          let file = `WQ1075Logger_SmartApp_${utilController.getDateTime().toString().replace(/-/g, '_').replace(/:/g, '.').replace(/.480Z/g, '')}.xlsx`
