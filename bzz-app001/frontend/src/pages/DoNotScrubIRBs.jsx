...
export default function DoNotScrubIrbs() {
  const entity = "donotscrubirbs"; // entity api name , don't change it
...
useEffect(() => {
  ...
  const logger = (value) => {
    request.create('/donotscrubirbsLogger', value)
  }
    ...
    },
    {
      ...
      render: (text, row) => { 
        return {
          children: (
            <div>
               { text ? text.split('T')[0].split('-')[1] + "-"  + text.split('T')[0].split('-')[2] + "-" + text.split('T')[0].split('-')[0] + "" + " " + text.split('T')[1].slice(0,8): ''  }
  ...
  const DATATABLE_TITLE = "Do not Scrub IRBs";
