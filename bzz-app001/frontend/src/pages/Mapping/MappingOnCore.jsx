...
import MappingDataTableModule from "@/modules/MappingOnCoreDataTableModule";
...
export default function MappingOnCore() {
  ...
  const entity = "mapping-oncore";
  const entity2 = "timepoint-oncore";
  ...
  const openingModal = async (  row,type, tab) => {
    ...
    let data = []
    if (tab.toLowerCase() == 'epic') {
      let {result} =await request.create(entity + "-"+ tab , {IRB: row['IRB'], Name: row['item']['TP_NAME']}, )
      data = result
    } else {
      let{result} =await request.create(entity + "-"+ tab , {IRB: row['IRB'], Name: row['item']['EPIC TP']}, )
      data = result
    }
  ...
  const panelTitle = "";
  const dataTableTitle = "Mapping OnCore";
  ...
  const onEditItem = async(values) => {
    if (subEntity == 'epic') {
      ...
      await request.update(entity+"-"+ subEntity, selectedId.IRB, {Notes: values['Notes'], Name: selectedId.item['EPIC TP']})
