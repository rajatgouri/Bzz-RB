...
const FlowchartDiagram = ({ data, edit }) => {
  ...
  let config = {
    ...
    data: {...DATA},
    onSave: async (value) => {
     await request.update('flowcharts', 0, {'1262Flowchart': JSON.stringify(value)});
...
const DemoFlowchart = ({data, update, onUpdate}) => {
  ...
  const fetchData = async() => {
    const {result, success, message} = await request.list('wq1262-flowchart');
...
export default function FlowchartPage() {
  ...
  const fetchData = async() => {
    ...
    setValue(resposne.result[0]['1262Flowchart'] ?  resposne.result[0]['1262Flowchart'] : {})        
