...
export default function Irb() {
  const entity = "nontherapeuticstudies"; // entity api name , don't change it
  ...
  const showEditModal = () => {
    ...
    editForm.setFieldsValue({IRB: data.IRB, Notes: data.Notes, Type: data.Type})
...
useEffect(() => {
  ...
  const onAddItem = async(values) => {
    ...
    if(response.success) {
      setIsEditModalVisible(false)
      logger({IRB: values.IRB, 'Status': 'Added'})
      reloading()
      addForm.resetFields()
      notification.success({message: "Successfully Added!", duration: 3});
    } 
  }

  const logger = (value) => {
    request.create('/ntstudiesLogger', value)
  }

  const onAddFailed = (errorInfo) => {
    notification.error({message: "Please eneter value!", duration: 3});
  };
  ...
  const addModal = {
    ...
    content : (
        <Form
        name="basic"
        style={{marginTop: "6px !important", width: "300px"}}
        labelCol={{ span: 6 }}
        onFinish={onAddItem}
      ...
      >
        ...
        >
            ...
            >
              ...
              <Select.Option value={'Non-Therapeutic'}>Non-Therapeutic</Select.Option>
  ...
  const updateTypeChange = (value, row) => {
    const { ID  } = row;

    const fieldsValue = { Type: value } ;
    dispatch(crud.update(entity, ID[0], fieldsValue));
    logger({IRB: values.IRB, 'Status': 'Update Type - '+ value })
        ...
        return {
          ...
          children: (
            <Select
              ...
              onSelect={(value) => updateTypeChange(value, row)}
            >
              <Select.Option value={'Therapeutic'}>Therapeutic</Select.Option>
              <Select.Option value={'Non-Therapeutic'}>Non-Therapeutic</Select.Option>
              <Select.Option value="null">&nbsp;</Select.Option>
  ...
  const onEdit = async(values) => {
    let response = await request.update(entity, selectedIRB, {IRB: values.IRB, Type: values.Type, Notes: values.Notes != null ?  values.Notes : "" , LastUpdated: currentDate});
    if(response.success) {
      setIsEditModalVisible(false)
      reloading()
      notification.success({message: "Sucessfully updated!", duration: 3})
      logger({IRB: values.IRB, 'Status': 'Edited'})
  ...
  const onDeleteIRB = async () => {
    ...
    notification.success({message: "Successfully deleted!",  duration: 3})
    logger({IRB: selectedIRB, 'Status': 'Deleted'})
  ...
  const DATATABLE_TITLE = "Non-Therapeutic Studies";
