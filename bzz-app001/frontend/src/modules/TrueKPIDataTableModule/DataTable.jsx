...
export default function TrueKPI({ config }) {
  ...
  let { entity, dataTableColumns, dataTableTitle, onhandleSave, openEditModal, openAddModal, getItems, reload, progressEntity, workEntity, onWorkSaved, onCopied, getFilterValue, showProcessFilters, userList, onRowMarked, height, className } = config;
  ...
  const newDataTableColumns = dataTableColumns.map((obj, i) => {
    ...
    if (obj.dataIndex == "Total Charges/CPT Count" ) {
      return ({
        ...obj,
        render: (text, row) => {
