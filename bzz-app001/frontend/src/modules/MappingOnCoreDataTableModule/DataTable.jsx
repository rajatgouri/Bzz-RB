...
export default function DataTable({ config }) {
  ...
  const onCopyPrevious = (index)  => {
    ...
    if(dataSource[index]['option'].filter((opt) => opt['SheetName'] == dataSource[index - 1].value ).length > 0) {
      ...
      dataSource[index]['item']['OnCore Arm'] = dataSource[index - 1].value 
  ...
  const onCopyPreviousTimepoint = (index)  => {
      ...
      dataSource2[index]['item']['OnCore TP'] = dataSource2[index - 1].value 
  ...
  const onChangeMCATimepoint = async (value, index) => {
    ...
    dataSource2[index]['item']['OnCore TP'] = value

    setDataSource2([...dataSource2])


    await request.create(entity2, {
      'EPIC TP': dataSource2[index]['item']['EPIC TP'],
      'IRB': dataSource2[index]['IRB'],
      'OnCore TP': value,
      'OnCore Arm': selectedTimepoint
    })
...
return (s)
          ...
          return {
            ...
            children: (
              <div >
                  ...
                  <span className="mr-10">
                   <img src={CheckImage} height="17px" weight="16px" style={{ filter: row['item']['OnCore TP'] ?  "hue-rotate(293deg)" : "grayscale(100%)", marginLeft: "1px", marginRightL :"20px" }}  />  
                  </span>
                  <span>
                  {row['item']['EPIC TP']}
                  </span>
  ...
  const onSaveARMs = async() => {
    ...
    let obj = dataSource.map((d) => {
      return {
          ...
          'IRB': d['IRB'],
          'OnCore Arm': d['item']['Selected MCA']
      }
    })
    setLoading(true)
  ...
  const onSaveTimepoints = async() => {
    ...
    let obj = dataSource2.map((d) => {
      return {
        'EPIC TP': d['item']['EPIC TP'],
        'IRB': d['IRB'],
        'OnCore TP': d['item']['OnCore TP'],
        'OnCore Arm': d['OnCore Arm']
      }
    })
