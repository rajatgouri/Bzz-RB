...
export default function KPIDataTable({ config }) {
  ...
  useEffect(() => {
    ...
    let range = items.map((item) => item['DataRange'])
    range = [...new Set(range)]; 
    ...
    let empIds =  users.map((u) => ({EMPID: u.EMPID, name: u.name}))
      ...
      range.map((r, index) => {
        let row = items.filter((item) => item['DataRange'] == r )
        ...
        let Obj = {}
          ...
          empIds.map((e, i) => {
            ...
            let r1 = row.filter((ro) => ro.EMPID == e.EMPID)
            
            Obj['ActionTimeStamp']=  r  
            Obj[ i + '-WQ5508Charges'] = (r1.reduce((a,b) => a + b['WQ5508ChargesProcessedAvg'],0))
            Obj[ i +'-WQ1075Charges'] = (r1.reduce((a,b) => a + b['WQ1075ChargesProcessedAvg'],0)),
            Obj[ i + '-WQ5508Accounts'] = (r1.reduce((a,b) => a + b['WQ5508AccountsProcessedAvg'],0))
            Obj[ i +'-WQ1075Accounts'] = (r1.reduce((a,b) => a + b['WQ1075AccountsProcessedAvg'],0))
            Obj[ i + '-WQ5508Amount'] = (r1.reduce((a,b) => a + b['WQ5508AmountRemovedAvg'],0))
            Obj[ i + '-WQ1075Amount'] = (r1.reduce((a,b) => a + b['WQ1075AmountRemovedAvg'],0)) 
            Obj['color'] = '600'
            Obj['style'] = 'italic'
          })
