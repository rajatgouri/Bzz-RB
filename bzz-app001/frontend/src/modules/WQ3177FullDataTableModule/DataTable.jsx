...
import { CaretDownOutlined, CloseOutlined, CopyOutlined, EditOutlined, EyeFilled, UnorderedListOutlined, ReloadOutlined, UploadOutlined, SettingOutlined, CloudDownloadOutlined, FileExcelOutlined, SortAscendingOutlined } from "@ant-design/icons";
...
import { WQ3177Details } from "@/socket";
...
export default function DataTable({ config }) {
  ...
  function copy(id, textToCopy, filter) {
  ...
  const newDataTableColumns = dataTableColumns.map((obj) => {
    ...
    if (obj.dataIndex == "Date of Service" || obj.dataIndex == "Date Created" || obj.dataIndex == "Min Days End Date" || obj.dataIndex == "Consented" || obj.dataIndex == "Enrolled - Active") {
      return ({
        ...obj,
        render: (text, row) => {
      ...
      return ({
        ...
        render: (text, row) => {
          return {
            ...
            children: (
              <div>
                {text} <CopyOutlined onClick={() => copy(row.ID, text, true)} />
              </div>
            )
          };
        },
  ...
  var { pagination, items, filters, sorters, colors, scrubIrb } = listResult;
...
WQ3177Details.subscribe(
  ...
  const getPercentage = (fullList = []) => {
    ...
    if (fullList) {
      ...
      const amount = inReview.map(li => li['Amount Due']).sort((a, b) => b - a).slice(0, 5)
      const amountReview = list.map(li => li['Amount Due']).sort((a, b) => b - a).slice(0, 10)
  ...
  const loadTable = () => {
    ...
    let filterValue
    let sortValue
    if (current.name == 'Bernadette' || current.Nickname == 'Beth') {
      filterValue = { Status: ['Review'], }

    } else if (current.managementAccess) {
      filterValue = { UserAssigned: [] ,Status: ['Done', 'Pending', 'Misc', 'Deferred', 'Review'],}
    } else  {
      filterValue = { UserAssigned: [current.name], Status: ['Review'], }
    }
    ...
    (async () => {
      const responseForFilterQuery = await request.list(entity + "-filters", { user: [current.Nickname] });
       await request.list(entity + "-full-list");
      getFullList(responseForFilterQuery.result, scrubIrb, filterValue)
    })()
              ...
              <Col style={{ width: "100%", marginTop: "10px" }}>
                ...
                <div>
                  ...
                  <Radio.Group value={process} onChange={onProcessChanged}>
                    ...
                    <Radio.Button value="Perm" className="mr-3 box-shadow" >No Scrub-Perm</Radio.Button>
